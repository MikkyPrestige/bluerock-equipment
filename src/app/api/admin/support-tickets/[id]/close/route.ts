...
export async function POST(
  ...
  const { data: ticket, error } = await adminSupabase
    ...
    .update({ status: 'closed', closed_by: 'admin' })
    .eq('id', id)
  ...
  after(async () => {
    ...
    const { error: emailErr } = await resend.emails.send({
      from: fromAddress,
      to: buyer.email,
      subject: `Ticket Closed — ${ticket.subject} — BlueRock Equipment Support`,
      html: `
<!DOCTYPE html>
<html>
...
<body style="font-family:Arial,Helvetica,sans-serif;color:#1a1a1a;max-width:560px;margin:0 auto;padding:32px 16px">
  <div style="background:#1a3a5c;padding:24px 32px;border-radius:8px 8px 0 0">
    ...
    <p style="font-size:11px;color:#b8962e;margin:4px 0 0;letter-spacing:2px;text-transform:uppercase">Support Ticket Closed</p>
  </div>
  <div style="background:#fff;border:1px solid #dde3ea;border-top:none;padding:32px;border-radius:0 0 8px 8px">
    <p style="font-size:14px;color:#4a4a4a;line-height:1.6;margin:0 0 24px">
      Your support ticket <strong>${ticket.subject}</strong> has been closed by our team.
    </p>
    ...
    if (emailErr) console.error('[admin/support-tickets/close] Resend error:', JSON.stringify(emailErr))
  })
