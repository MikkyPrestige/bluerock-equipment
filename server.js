...
function fmtUsd(amount) {
  ...
  if (!n) return '<span style="color:#9ca3af;font-size:12px">TBD</span>'
  return '$' + n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
}

function buildProformaHTML({ quoteId, issueDate, validUntil, buyer, machine, pricing }) {
  const machinePrice = Number(pricing.machinePrice) || 0
  const freightEst = Number(pricing.freightEstimate) || 0
  const customsFee = Number(pricing.customsFee) || 0
  const total = Number(pricing.total) || machinePrice
  const refId = String(quoteId).substring(0, 8).toUpperCase()
...
.terms{padding:18px 36px;background:#fff}
...
<div class="hdr">
  ...
  <div class="doc-info">
    ...
    <div class="doc-num">No. PRF-${esc(refId)}</div>
    <div class="doc-date">Issued: ${esc(issueDate)}</div>
...
<div class="parties">
  ...
  <div class="party">
    ...
    <div class="pname">${esc(buyer.company || buyer.name || 'Buyer')}</div>
    <div class="pdetail">
      ${buyer.address ? esc(buyer.address) + '<br/>' : ''}
      ${buyer.portOfDischarge
        ? `Port of Discharge: <span class="phighlight">${esc(buyer.portOfDischarge)}</span>`
        : ''}
    </div>
  </div>
</div>
...
<div class="machine-sec">
  ...
  <div class="machine-name">${esc(machine.name || `${machine.year} ${machine.brand} ${machine.model}`)}</div>
  <div class="specs-grid">
    <div class="spec">
      <div class="spec-label">Brand</div>
      <div class="spec-val">${esc(machine.brand)}</div>
    </div>
    <div class="spec">
      <div class="spec-label">Model</div>
      <div class="spec-val">${esc(machine.model)}</div>
    </div>
    <div class="spec">
      <div class="spec-label">Year</div>
      <div class="spec-val">${esc(String(machine.year))}</div>
    </div>
    ${machine.serialNumber
      ? `<div class="spec"><div class="spec-label">Serial No.</div><div class="spec-val">${esc(machine.serialNumber)}</div></div>`
      : ''}
    ${machine.engineHours
      ? `<div class="spec"><div class="spec-label">Engine Hours</div><div class="spec-val">${Number(machine.engineHours).toLocaleString()} hrs</div></div>`
      : ''}
    ${machine.category
      ? `<div class="spec"><div class="spec-label">Category</div><div class="spec-val">${esc(machine.category)}</div></div>`
      : ''}
  </div>
</div>
...
<div class="pricing-sec">
  ...
  <table>
    <thead>
      <tr>
        <th>Description</th>
        <th class="ra">Amount (USD)</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td class="desc">
          ${esc(machine.year)} ${esc(machine.brand)} ${esc(machine.model)}
          <div class="note">Ex-yard &middot; ${esc(machine.category || '')}</div>
        </td>
        <td class="amt">${fmtUsd(machinePrice)}</td>
      </tr>
      <tr>
        <td class="desc">
        ...
        <td class="amt">${fmtUsd(freightEst)}</td>
      </tr>
      <tr>
        <td class="desc">
        ...
        <td class="amt">${fmtUsd(customsFee)}</td>
      </tr>
      <tr class="total-row">
        <td class="desc">TOTAL ESTIMATED COST</td>
        <td class="amt">${fmtUsd(total)}</td>
      </tr>
    </tbody>
  </table>
</div>
...
<div class="payment-bar">
  ...
  <div class="pblock">
    ...
    <div class="pbval">${esc(buyer.portOfDischarge || 'To be confirmed')}</div>
  </div>
  <div class="pblock">
...
<div class="sig-row">
  ...
  <div class="sig-block">
    <div class="sig-line">
      ...
      <div class="sig-name">${esc(buyer.company || buyer.name || '')}</div>
    </div>
  </div>
</div>
...
<div class="footer">
  ...
  <span>PRF-${esc(refId)} &middot; ${esc(issueDate)}</span>
</div>
...
function overallAssessment(wearAnalysis) {
  const vals = Object.values(wearAnalysis || {})
...
function buildInspectionHTML({ machine, inspectionDate }) {
  const {
    name, brand, model, year, serialNumber, category, useCase,
    engineHours, hoursSinceService, yardCity, yardCountry,
    wearAnalysis, description,
  } = machine

  const wear = wearAnalysis || {}
...
.badge-150 small{display:block;font-size:9px;font-weight:400;margin-top:2px;letter-spacing:1px;text-transform:uppercase;opacity:0.85}
...
<div class="hdr">
  ...
  <div style="text-align:right">
    <div class="badge-150">
      ...
      <small>Yard Inspection Completed</small>
    </div>
    <div style="font-size:10px;color:#8aacc8;margin-top:8px">Inspection Date: ${esc(inspectionDate)}</div>
  </div>
</div>

<div class="id-block">
  <div class="id-main">
    <div class="machine-title">${esc(name || `${year} ${brand} ${model}`)}</div>
    <div class="machine-sub">${esc(category || '')}${useCase ? ' &middot; ' + esc(useCase) : ''}</div>
  </div>
  <div class="id-meta">
    <div class="meta-item">
      <div class="meta-label">Brand</div>
      <div class="meta-val">${esc(brand)}</div>
    </div>
    <div class="meta-item">
      <div class="meta-label">Model</div>
      <div class="meta-val">${esc(model)}</div>
    </div>
    <div class="meta-item">
      <div class="meta-label">Year</div>
      <div class="meta-val">${esc(String(year))}</div>
    </div>
    ${serialNumber ? `<div class="meta-item"><div class="meta-label">Serial No.</div><div class="meta-val">${esc(serialNumber)}</div></div>` : ''}
    <div class="meta-item">
      <div class="meta-label">Engine Hours</div>
      <div class="meta-val">${engineHours ? Number(engineHours).toLocaleString() + ' hrs' : 'N/A'}</div>
    </div>
    ${hoursSinceService ? `<div class="meta-item"><div class="meta-label">Hrs Since Service</div><div class="meta-val">${Number(hoursSinceService).toLocaleString()} hrs</div></div>` : ''}
    <div class="meta-item">
      <div class="meta-label">Yard Location</div>
      <div class="meta-val">${esc(yardCity)}, ${esc(yardCountry)}</div>
    </div>
  </div>
</div>
...
<div class="overall" style="background:${overall.bg};border:1px solid ${overall.text}22">
  <div>
    ...
    <div class="overall-val" style="color:${overall.text}">${esc(overall.label)}</div>
  </div>
  <div style="margin-left:auto;font-size:10px;color:#6c757d">
    ${Object.keys(wear).length} components inspected
  </div>
</div>
...
<div class="desc-sec">
  ...
  <div class="desc-text">${esc(description)}</div>
</div>` : ''}

<div class="cert">
  <div class="cert-text">
    This inspection report documents the physical condition of the listed equipment as assessed by BlueRock Equipment's yard inspection team.
    All component ratings reflect visual inspection and operational testing completed on the inspection date above.
    This report is valid for 90 days from the inspection date.
  </div>
...
<div class="footer">
  <span>BlueRock Equipment &middot; Yard Inspection Report &middot; ${esc(name || `${brand} ${model}`)}</span>
  <span>Document generated: ${esc(inspectionDate)}</span>
</div>
...
async function getChromePath() {
  ...
  // Lambda / cloud: download binary from CHROMIUM_DOWNLOAD_URL env var
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/api/pdf/proforma' && req.method === 'POST') {
    try {
      const data = await readBody(req)
      const html = buildProformaHTML(data)
      const pdf = await generatePDF(html)
      const refId = String(data.quoteId || 'doc').substring(0, 8).toUpperCase()
      res.writeHead(200, {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="proforma-${refId}.pdf"`,
      ...
      res.end(JSON.stringify({ error: 'PDF generation failed', detail: err.message }))
    }
    return
  }
  ...
  if (req.url === '/api/pdf/inspection-report' && req.method === 'POST') {
    try {
      ...
      const pdf = await generatePDF(html)
      const name = esc(data.machine?.brand || 'machine').substring(0, 20)
      res.writeHead(200, {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="inspection-${name}.pdf"`,
        'Content-Length': pdf.length,
      })
      res.end(pdf)
    } catch (err) {
