...
import DocumentVaultTable, { type DocumentRow } from '@/components/dashboard/DocumentVaultTable'
...
export default async function DocumentVaultPage() {
  ...
  const { data: rawDocs, count } = await adminSupabase
    .from('documents')
    .select('id, quote_id, document_type, version, superseded_at, created_at', { count: 'exact' })
    .eq('buyer_id', user.id)
      ...
      <div className="bg-navy-900 border-b border-white/8 px-6 py-5">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-2 text-xs text-white/30 mb-2">
            ...
            <span>/</span>
            <span className="text-white/55">Document Vault</span>
          </div>
          <div className="flex items-center gap-3">
            <h1 className="font-display text-2xl sm:text-3xl font-bold text-white">Document Vault</h1>
            {(count ?? 0) > 0 && (
          ...
          <p className="text-white/25 text-xs mt-2">
            Active trade documents only · Download links expire after 1 hour for security
          </p>
        </div>
      </div>

      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8">
        <DocumentVaultTable documents={documents} />
      </main>
